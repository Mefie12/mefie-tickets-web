...
export function ComplimentaryDistributorManager({ eventId, program, products, onProgramChange }: { eventId: number; program: ComplimentaryProgram; products: Product[]; onProgramChange: () => void }) {
  ...
  function confirmManage(id: number, action: "revoke" | "close", title: string, body: string, confirmLabel: string) {
    modals.openConfirmModal({
      title, centered: true,
      children: <Text size="sm">{body}</Text>,
      labels: { confirm: confirmLabel, cancel: "Cancel" },
      confirmProps: { color: "red" },
      onConfirm: () => manage.mutate({ id, action }),
    });
  }
  function submit() {
  ...
  return <Stack gap="lg">
    ...
    {allocations.data?.length ? <Accordion variant="separated">{allocations.data.map((allocation) => <Accordion.Item key={allocation.id} value={String(allocation.id)}><Accordion.Control><Group justify="space-between" pr="md"><Stack gap={0}><Text fw={600}>{allocation.distributor.name}{allocation.distributor.company ? ` · ${allocation.distributor.company}` : ""}</Text><Text size="xs" c="dimmed">{allocation.distributor.email}</Text></Stack><Badge color={statusColor(allocation.status)}>{allocation.status}</Badge></Group></Accordion.Control><Accordion.Panel><Stack>
      ...
      <Text size="xs" c="dimmed">Deadline: {new Date(allocation.distribution_deadline_at).toLocaleString()}</Text><Group justify="flex-end">{allocation.status === "PENDING" && <><Button size="xs" variant="light" leftSection={<IconMailForward size={14} />} onClick={() => manage.mutate({ id: allocation.id, action: "resend" })}>Resend</Button><Button size="xs" variant="light" color="red" onClick={() => confirmManage(allocation.id, "revoke", "Revoke this invitation?", "The distributor will no longer be able to accept it.", "Revoke")}>Revoke</Button></>}{allocation.status === "ACTIVE" && <><Button size="xs" variant="light" color="yellow" onClick={() => manage.mutate({ id: allocation.id, action: "suspend" })}>Suspend</Button><Button size="xs" variant="light" color="red" onClick={() => confirmManage(allocation.id, "close", "Close this allocation?", "Any unused balance is returned to the pool. This cannot be undone.", "Close & return unused")}>Close & return unused</Button></>}{allocation.status === "SUSPENDED" && <><Button size="xs" variant="light" onClick={() => manage.mutate({ id: allocation.id, action: "resume" })}>Resume</Button><Button size="xs" variant="light" color="red" onClick={() => confirmManage(allocation.id, "close", "Close this allocation?", "Any unused balance is returned to the pool. This cannot be undone.", "Close & return unused")}>Close & return unused</Button></>}</Group>
