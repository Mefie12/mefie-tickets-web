...
function AccountForm({ signup }: { signup: boolean }) {
  ...
  const terms = useQuery({ queryKey: ["platform-document", "terms-of-use"], queryFn: () => getPublicPlatformDocument("terms-of-use") });
  const privacy = useQuery({ queryKey: ["platform-document", "privacy-policy"], queryFn: () => getPublicPlatformDocument("privacy-policy") });
  const needsDetails = step === "complete" || (signup && step === "details");
  const documents = [terms.data?.version_id, privacy.data?.version_id].filter((id): id is number => id !== undefined);
      ...
      <Stack>
        ...
        {step === "code" ? <><PinInput length={6} type="number" oneTimeCode value={code} onChange={setCode} aria-label="Email verification code"/><Button type="submit" loading={busy} disabled={code.length !== 6}>Verify email</Button><Button variant="subtle" disabled={busy} onClick={() => submit("request")}>Send another code</Button><Button variant="subtle" onClick={() => { setStep("details"); setCode(""); setProof(null); }}>Change email</Button></> : <>
          ...
          {needsDetails && <><TextInput label="First name" required autoComplete="given-name" {...form.getInputProps("first_name")}/><TextInput label="Last name" required autoComplete="family-name" {...form.getInputProps("last_name")}/><PhoneInput label="Phone" required {...form.getInputProps("phone")}/>
            <Text size="xs" c="dimmed">By creating an account, you agree to the {terms.data ? <TermsAndConditionsLink document={terms.data} pdfUrl="/api/public/platform-documents/terms-of-use/pdf" label="Terms of Use"/> : "Terms of Use"} and acknowledge the {privacy.data ? <TermsAndConditionsLink document={privacy.data} pdfUrl="/api/public/platform-documents/privacy-policy/pdf" label="Privacy Policy"/> : "Privacy Policy"}.</Text>
            {(terms.isError || privacy.isError) && <Alert color="red">Unable to load account documents. Please reload to try again.</Alert>}
          </>}
          <Button type="submit" loading={busy} disabled={needsDetails && (terms.isPending || privacy.isPending || terms.isError || privacy.isError)}>{step === "complete" ? "Create account" : "Continue with email"}</Button>
        </>}
