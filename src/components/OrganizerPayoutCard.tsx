 ...
 */
export function OrganizerPayoutCard({ summary }: { summary: OrganizerPayoutSummary }) {
  return (
      ...
      <Stack gap="md">
        ...
        <Group grow>
          <Stack gap={0}>
            ...
            <Text fw={600}>${summary.collected}</Text>
          </Stack>
          <Stack gap={0}>
            <Text size="xs" c="dimmed">
              Released
            </Text>
            <Text fw={600}>${summary.released}</Text>
          </Stack>
          <Stack gap={0}>
            <Text size="xs" c="dimmed">
              Balance
            ...
            <Stack gap="xs">
              {summary.history.map((release) => (
                <Group key={release.id} justify="space-between" align="flex-start">
                  <Stack gap={0}>
                    <Text size="sm">${release.amount}</Text>
                    {release.note && (
                      <Text size="xs" c="dimmed">
                        {release.note}
                      </Text>
